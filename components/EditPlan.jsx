...
export default EditPlan = ({route}) => {
    ...
    return (
        <View>
            <Text>Day: {plan.day}</Text>
            <TextInput placeholder={plan.day} onChangeText={(txt) => {
                setDay(txt)
                console.log(day)
                }}></TextInput>
            ...
            <Button title='Save plan' onPress={() => updatePlan(day, exercises, plan.userid, plan.id)}></Button>
        </View>
    )
}
...
const updatePlan = async (day, exercises, userid, id) => {
    await updateDoc(doc(database, "fitnessPlan", id),{
        day: day,
        userid: userid,
        exercises: exercises
    })
